...
void main() {
    ...
    vec4 tan   = normalize(p_);
    vec4 bnorm = normalize(cross(p_, p_2));
    vec4 norm  = normalize(cross(bnorm, tan));

    mat4 basis = mat4(norm, bnorm, tan, origin);
    ...
    vec4 pos = basis * inverse(basis_og) * aPos;
    vec4 posCam = MV * aPos;
